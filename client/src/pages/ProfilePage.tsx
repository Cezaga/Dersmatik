...
import { User, Award, LogOut, Save, Flame, Zap, Target, BookOpen } from 'lucide-react';
...
export default function ProfilePage() {
  ...
  const [form, setForm] = useState({ displayName: '', targetRank: '', targetDepartment: '', dailyGoalMinutes: 120, dailyGoalQuestions: 50 });
    ...
    <div className="space-y-6 max-w-2xl mx-auto">
      ...
      <div className="bg-dark-800 rounded-2xl p-6 border border-dark-700 space-y-4">
        <div className="flex items-center justify-between">
          ...
          {!editing ? (
            <button onClick={() => setEditing(true)} className="text-sm text-primary-400">Duzenle</button>
          ) : (
        ...
        <div className="space-y-3">
          <div>
            <label className="text-xs text-dark-400">Gorünen Ad</label>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input type="number" placeholder="orn: 10000" value={form.targetRank} onChange={e => setForm(p => ({ ...p, targetRank: e.target.value }))} disabled={!editing}
                className="w-full px-4 py-2 bg-dark-700 border border-dark-600 rounded-xl text-white disabled:opacity-50 mt-1" />
            </div>
            <div>
              <label className="text-xs text-dark-400">Hedef Bolum</label>
              <input placeholder="orn: Tıp" value={form.targetDepartment} onChange={e => setForm(p => ({ ...p, targetDepartment: e.target.value }))} disabled={!editing}
                className="w-full px-4 py-2 bg-dark-700 border border-dark-600 rounded-xl text-white disabled:opacity-50 mt-1" />
            </div>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-xs text-dark-400">Gunluk Hedef (dakika)</label>
            ...
            <div>
              <label className="text-xs text-dark-400">Gunluk Hedef (soru)</label>
      ...
      <div className="bg-dark-800 rounded-2xl p-6 border border-dark-700">
        ...
        <div className="grid grid-cols-3 md:grid-cols-4 gap-3">
          {badges.map(b => (
            <div key={b.id} className={`p-3 rounded-xl text-center transition ${b.earned ? 'bg-primary-600/10 border border-primary-500/30' : 'bg-dark-700/50 opacity-40'}`}>
              <div className="text-2xl mb-1">{b.icon}</div>
              <div className="text-xs font-medium">{b.name}</div>
              <div className="text-[10px] text-dark-400 mt-1">{b.description}</div>
            </div>
          ))}
        </div>
      </div>
