...
import { Plus, RotateCcw, ChevronLeft, BookOpen, X } from 'lucide-react';
...
export default function FlashcardsPage() {
  ...
  const loadCards = async (deck: Deck) => {
    ...
    try {
      ...
      if (c.length === 0) {
        setMsg('Bu destede tekrar edilecek kart yok. Yeni kart ekleyin veya daha sonra tekrar deneyin.');
      ...
      setMsg('Kartlar yüklenirken hata oluştu.');
  ...
  const createDeck = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setMsg('Deste oluşturuldu!');
      setTimeout(() => setMsg(''), 3000);
    } catch {
      setMsg('Deste oluşturulurken hata oluştu.');
      setTimeout(() => setMsg(''), 3000);
    }
    setLoading(false);
  };
  ...
  const addCard = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setMsg('Kart eklenirken hata oluştu.');
      setTimeout(() => setMsg(''), 3000);
    }
  };
  ...
  const reviewCard = async (quality: number) => {
    ...
    try {
      ...
      if (currentIndex < cards.length - 1) {
        ...
        setCards([]);
        setSelectedDeck(null);
        setMsg('Tüm kartları tamamladınız! +XP kazandınız.');
        setTimeout(() => setMsg(''), 3000);
      }
    } catch {}
  };
    ...
    <div className="space-y-6">
      ...
      {showNewDeck && (
        <form onSubmit={createDeck} className="bg-dark-800 rounded-2xl p-4 border border-dark-700 space-y-3 animate-slide-up">
          ...
          <input placeholder="Açıklama (opsiyonel)" value={newDeck.description} onChange={e => setNewDeck(p => ({ ...p, description: e.target.value }))}
            className="w-full px-4 py-3 bg-dark-700 border border-dark-600 rounded-xl text-white" />
          <button type="submit" disabled={loading} className="w-full py-3 bg-primary-600 rounded-xl font-semibold disabled:opacity-50 hover:bg-primary-500 transition">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        {decks.map(deck => (
          <div key={deck.id} className="bg-dark-800 rounded-xl p-4 border border-dark-700 hover:border-dark-500 transition overflow-hidden">
            <div className="flex items-center justify-between mb-2">
              <h3 className="font-semibold truncate">{deck.title}</h3>
              <span className="text-xs text-dark-400 shrink-0 ml-2">{deck.card_count} kart</span>
            </div>
            {deck.description && <p className="text-sm text-dark-400 mb-3 line-clamp-2">{deck.description}</p>}
            <div className="flex gap-2">
              ...
              <button onClick={() => { setSelectedDeck(deck); setShowAddCard(true); }} className="px-3 py-2 bg-dark-700 rounded-lg text-sm text-dark-300 hover:bg-dark-600">
                <Plus size={14} />
              </button>
            </div>
          </div>
        ...
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
          <form onSubmit={addCard} className="bg-dark-800 rounded-2xl p-6 border border-dark-700 max-w-md w-full space-y-4">
            <div className="flex items-center justify-between">
              ...
              <button type="button" onClick={() => { setShowAddCard(false); setSelectedDeck(null); }}><X size={18} className="text-dark-400" /></button>
            </div>
            <textarea placeholder="Ön Yüz (Soru)" value={newCard.front} onChange={e => setNewCard(p => ({ ...p, front: e.target.value }))} required
              className="w-full px-4 py-3 bg-dark-700 border border-dark-600 rounded-xl text-white h-24 resize-none" />
            <textarea placeholder="Arka Yüz (Cevap)" value={newCard.back} onChange={e => setNewCard(p => ({ ...p, back: e.target.value }))} required
              className="w-full px-4 py-3 bg-dark-700 border border-dark-600 rounded-xl text-white h-24 resize-none" />
            <div className="flex gap-3">
              <button type="button" onClick={() => { setShowAddCard(false); setSelectedDeck(null); }} className="flex-1 py-2 bg-dark-700 rounded-xl">İptal</button>
              <button type="submit" className="flex-1 py-2 bg-primary-600 rounded-xl font-semibold hover:bg-primary-500 transition">Ekle</button>
            </div>
          </form>
        </div>
      )}

      {decks.length === 0 && !showNewDeck && <p className="text-center text-dark-400 py-8">Henüz flashcard destesi oluşturulmamış</p>}
    </div>
  );
}
